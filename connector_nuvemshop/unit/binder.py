...
class NuvemshopModelBinder(NuvemshopBinder):
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.nuvemshop_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.nuvemshop_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Nuvemshop! Prevent False values
        ...
        # avoid to trigger the export when we modify the `nuvemshop_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``nuvemshop.product.product`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``nuvemshop.product.product``,
