...
class StockQuant(models.Model):
    ...
    def create(self, vals):
        ...
        ps_locations = location_obj.get_nuvemshop_stock_locations()
        quant = super(StockQuant, self).create(vals)
        if quant.location_id in ps_locations:
            quant.product_id.update_nuvemshop_qty()
        return quant
    ...
    def unlink(self):
        ps_locations = self.env['stock.location'].\
            get_nuvemshop_stock_locations()
        self.filtered(lambda x: x.location_id in ps_locations).mapped(
            'product_id').update_nuvemshop_qty()
